...
"""Scrub private paths and inject a validated release version."""
...
firmware_version = os.environ.get(
    "CHARADOCK_RLCD_FIRMWARE_VERSION", "0.6.0"
)
if not re.fullmatch(r"[0-9A-Za-z][0-9A-Za-z._-]{0,31}", firmware_version):
    raise ValueError("invalid CHARADOCK_RLCD_FIRMWARE_VERSION")
env.Append(
    CPPDEFINES=[
        ("CHARADOCK_RLCD_FIRMWARE_VERSION", f'\\"{firmware_version}\\"')
    ]
)
